...
| `broker.py` | The reference `InMemoryCredentialBroker` (§8 / ZU-CD-7/8, ZU-AUDIT-5). **Trusted because it holds the secret harness-side:** it enforces scope/payee-allowlist/per-use/cumulative(`incr_if_below`)/TTL/revocation mechanically, calls the `Instrument` on a full allow only, and binds every use to its consent on the chain. The secret never crosses back to the policy (the policy holds an opaque `Grant.id`). The reference instrument is FAKE; a real issuer/vault is a plugin behind the `Instrument` port. |
| `instruments.py` | The reference FAKE instruments (`FakeCardInstrument`/`FakeVaultInstrument`) — the in-memory doubles that hold the "secret" behind the `Instrument` boundary for the offline proofs. Trusted as the secret-holder in the reference build; a production issuer/vault is a plugin (not in the core). NO payment SDK, NO network. |
| `broker_gate.py` | The reference `BrokerGate` (§8 wiring): a thin `InvocationGate` that maps a high-consequence broker use (amount > `scope.requires_human_over`, a new payee — computed harness-side from the Grant + literal call args, never policy self-report) to `Verdict(kind="human")`, routing to the existing `_pause_for_human`. `fail_closed_on_crash` so a crashed money-gate fails closed. |
...
| `credential_brokers` (`zu.credential_brokers`) | `CredentialBroker` / `Instrument` (§8) | The broker holds the secret **harness-side** (behind the `Instrument`) and exposes only scoped, time-boxed, revocable, audited capabilities; the policy holds an opaque `Grant.id`, never the secret. Refuses any use outside scope/limits/TTL/revocation (ZU-CD-8); binds every use to its consent on the chain (ZU-AUDIT-5). The `Instrument` is the pluggable issuer/vault seam — a real issuer is a plugin here, **never** in the core. The broker is part of the trusted base *because* it holds the secret; for the strongest boundary it runs out-of-process (a `Channel` over `rpc.py`) so the secret lives in a separate process/uid. |
