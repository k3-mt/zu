...
from zu_core.track import REPLAY_JITTER_MAX_MS
...
class ReplayConfig(BaseModel):
    ...
    * ``jitter_max_ms`` — the most extra delay (ms) added to a replayed step,
      reached as the track nears 100%. Humanises a LIVE run's pacing (each step
      gets a random delay scaling upward with progress through the track); 0
      disables it. Forced off for ``--offline`` so iteration stays instant."""

    budget: Budget | None = None
    finish_model: str | None = None
    jitter_max_ms: int = REPLAY_JITTER_MAX_MS
